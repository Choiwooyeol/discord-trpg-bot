...
export class Application {
  ...
  async _create(event) {
    ...
    const session=this.engine.create({guildId:event.guildId,threadId:thread.id,hostId:event.userId,tone:String(event.value?.tone||'친구들과 즐기는 판타지 모험').slice(0,200),scenarioId:event.value?.scenario,minPartySize:event.solo===true?1:this.config.minPartySize});
